...
class L515_Node(object):
    ...
    def callback(self, ros_rgb, ros_depth):
        ...
        depth_frame = self.br.imgmsg_to_cv2(ros_depth, desired_encoding="passthrough")
        rgb_frame = self.br.imgmsg_to_cv2(ros_rgb, "bgr8")

        #depth_frame = np.array(depth_frame, dtype=np.float32)

        #resize image
        depth_frame = imutils.resize(depth_frame, width=1024)
        rgb_frame = imutils.resize(rgb_frame, width=1024)
